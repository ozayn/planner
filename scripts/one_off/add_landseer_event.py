#!/usr/bin/env python3
"""
Add the Sir Edwin Landseer Finding Awe event to the database
"""
import os
import sys
        ...
        if existing:
            ...
            try:
                ...
                if event_data.get('start_time'):
                    try:
                        existing.start_time = datetime.fromisoformat(event_data['start_time']).time()
                    except Exception as e:
                        print(f"   ⚠️  Could not update start_time: {e}")
                if event_data.get('end_time'):
                    try:
                        existing.end_time = datetime.fromisoformat(event_data['end_time']).time()
                    except Exception as e:
                ...
                if hasattr(existing, 'registration_opens_time') and event_data.get('registration_opens_time'):
                    try:
                        existing.registration_opens_time = datetime.fromisoformat(event_data['registration_opens_time']).time()
                    except Exception as e:
...
if __name__ == '__main__':
    try:
        print("=" * 80)
        print("Adding Landseer Finding Awe Event to Database")
        print("=" * 80)
        success = add_event()
        if success:
            print("\n" + "=" * 80)
