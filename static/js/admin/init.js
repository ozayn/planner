...
async function startACFDCScraping() {
    showScrapingProgressModal('Austrian Cultural Forum Washington (ACF DC)', false);

    try {
        const response = await fetch('/api/admin/scrape-acfdc-dc', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        ...
        console.error('ACF DC scraping error:', error);
        updateScrapingProgress({ percentage: 0, message: `❌ Error: ${error.message}`, error: true });
        updateScrapingStatus(`❌ Error: ${error.message}`, 'error');
        onScrapingComplete();
    }
...
async function startUniversityParkLibraryScraping() {
    showScrapingProgressModal('University Park Library', false);
    try {
        const response = await fetch('/api/admin/scrape-university-park-library', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        if (!response.ok) {
        ...
        if (result.success) {
            ...
            updateScrapingProgress({ percentage: 0, message: `❌ Error: ${result.error || 'Unknown error'}`, error: true });
            updateScrapingStatus(`❌ Error: ${result.error || 'Unknown error'}`, 'error');
            onScrapingComplete();
        }
    } catch (error) {
        console.error('University Park Library scraping error:', error);
        updateScrapingProgress({ percentage: 0, message: `❌ Error: ${error.message}`, error: true });
        updateScrapingStatus(`❌ Error: ${error.message}`, 'error');
        onScrapingComplete();
    }
...
/** Shared admin flow for Meetup group scrapers (progress modal, no scraping_progress.json polling). */
async function runMeetupGroupScrape(displayName, apiPath) {
    showScrapingProgressModal(displayName, false);
    ...
    try {
        ...
        console.error(`${displayName} Meetup scraping error:`, error);
        updateScrapingProgress({ percentage: 0, message: `❌ Error: ${error.message}`, error: true });
        updateScrapingStatus(`❌ Error: ${error.message}`, 'error');
        onScrapingComplete();
    }
...
async function startDCUrbanWalkersScraping() {
    return runMeetupGroupScrape('DC Urban Walkers (Meetup)', '/api/admin/scrape-dc-urban-walkers');
}
...
async function startTulipDayScraping() {
    showScrapingProgressModal('Tulip Day Washington');
    
    try {
        const response = await fetch('/api/admin/scrape-tulipday', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
...
async function startHammerScraping() {
    showScrapingProgressModal('Hammer Museum', false);

    try {
        const response = await fetch('/api/admin/scrape-hammer', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        ...
        console.error('Hammer Museum scraping error:', error);
        updateScrapingProgress({ percentage: 0, message: `❌ Error: ${error.message}`, error: true });
        updateScrapingStatus(`❌ Error: ${error.message}`, 'error');
        onScrapingComplete();
    }
...
async function startDeYoungScraping() {
    showScrapingProgressModal('de Young Museum', false);

    try {
        const response = await fetch('/api/admin/scrape-deyoung', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        ...
        console.error('de Young Museum scraping error:', error);
        updateScrapingProgress({ percentage: 0, message: `❌ Error: ${error.message}`, error: true });
        updateScrapingStatus(`❌ Error: ${error.message}`, 'error');
        onScrapingComplete();
    }
